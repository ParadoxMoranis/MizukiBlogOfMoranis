...
describe("Default image loading boundary", () => {
	...
	it("ships the configured local music covers while leaving remote covers dynamic", async () => {
		const coverFiles = [
			...musicConstants.matchAll(/coverFile:\s*string|song\([^\n]*?"([^"]+\.(?:jpg|png|webp))"/g),
		]
			.map((match) => match[1])
			.filter(Boolean);
		assert.ok(coverFiles.length >= 19);
		assert.doesNotMatch(musicCoverSource, /fetchpriority="high"/);

		for (const name of coverFiles) {
			const path = new URL(`../public/assets/music/cover/${name}`, import.meta.url);
			assert.ok(fileURLToPath(path));
			await readFile(path);
		}
	});
