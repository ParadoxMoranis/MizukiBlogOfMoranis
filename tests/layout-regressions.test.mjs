...
const postCardSource = await readFile(
	new URL("../src/components/features/posts/PostCard.astro", import.meta.url),
	"utf8",
);
...
const translateButtonSource = await readFile(
	new URL("../src/components/widget/TranslateButton.svelte", import.meta.url),
	"utf8",
);
...
const translationSources = await Promise.all(
	["en", "zh_CN", "zh_TW", "ja"].map((lang) =>
		readFile(
			new URL(`../src/i18n/languages/${lang}.ts`, import.meta.url),
			"utf8",
		),
	),
);
...
describe("Preserved site UI regressions", () => {
	it("keeps the legacy Neo post-card hierarchy on the upgraded template", () => {
		assert.match(postCardSource, /card-base neo-post-card/);
		assert.doesNotMatch(postCardSource, /showOnlyBasicMeta=\{true\}/);
		assert.doesNotMatch(postCardSource, /showWordCount=\{true\}/);
		assert.match(postCardSource, /remarkPluginFrontmatter\.words/);
		assert.match(postCardSource, /remarkPluginFrontmatter\.minutes/);
	});

	it("keeps the old homepage free of the upstream category bar", () => {
		assert.match(siteConfigSource, /categoryBar:\s*\{\s*enable:\s*false,/);
	});

	it("keeps the lazy-loaded translation control in the upgraded navbar", () => {
		assert.match(navbarSource, /import TranslateButton/);
		assert.match(navbarSource, /<TranslateButton client:only="svelte"/);
		assert.match(translateButtonSource, /id="translate-switch"/);
		assert.match(translateButtonSource, /window\.loadTranslateScript/);
		assert.match(layoutSource, /script\.src = "\/translate\.js"/);
		assert.match(layoutSource, /document\.addEventListener\("swup:page:view"/);
	});
});
...
describe("Global style loading regressions", () => {
	...
	it("loads Markdown styles from the Markdown wrapper", () => {
		...
		assert.match(globalStyleCheckSource, /\.custom-md/);
		assert.match(globalStyleCheckSource, /\.custom-md \.image-grid/);
		assert.match(globalStyleCheckSource, /--neo-accent:/);
	});
});
...
describe("Page layout variant regressions", () => {
	...
	it("keeps the wide-screen post layout configurable and pre-painted", () => {
		assert.match(
			siteConfigSource,
			/ultrawidePostLayout:\s*\{[\s\S]*?enable:\s*false[\s\S]*?allowSwitch:\s*false/,
			"the personalized UI keeps wide-screen expansion disabled by default",
		);
		assert.match(
			layoutSource,
